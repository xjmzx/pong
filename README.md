...
```
make deb
sudo apt install ./dist/pong_0.4.6_all.deb
```
...
2. `make deb && sudo apt install ./dist/pong_0.4.6_all.deb` — or `make deps && make install PREFIX=$HOME/.local` for a user-space install
