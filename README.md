...
```
make deb
sudo apt install ./dist/pong_0.4.5_all.deb
```
...
```
pip install pygame icalendar recurring-ical-events certifi pyinstaller
...
2. `make deb && sudo apt install ./dist/pong_0.4.5_all.deb` — or `make deps && make install PREFIX=$HOME/.local` for a user-space install
