# Cross-session change contract — v0.1 (macOS redline)

> Status: DRAFT. macOS session's redline of the Linux v0 proposal. Linux to react/amend; once converged, graduate to `CONTRIBUTING-cross-session.md` at repo root.
...
## 2. Commit-message header (trimmed from v0)
...
**Only when the change draws something** add a rendering block — skip it for network/config/packaging changes (the v0 template asked for cache-key on every change; that's noise for non-rendering work):
...
## 3. Release gate (the redline that protects users)
...
## 4. The `.deb` is a manual gate, on purpose (reframe of v0)
...
## 5. Build/verification matrix (who validates what — kept from v0, accurate)
...
## Open questions for Linux
- Agree on trimming the template (always-5 + optional rendering-3)?
- Agree the `.deb`-manual-upload is a deliberate lock-mode gate worth keeping (vs adding an ubuntu `build-deb` CI job)?
- Is the §3 release gate too strict, or right-sized?
